# PRD-50 — PDA Family (Photon Distribution Analysis)

Status: Draft · Parent: PRD-49 (§6 Phase 1, first target) · Type: Feature
Related: PRD-38 (model/view-spec split), PRD-40 (declarative editors),
PRD-04 (burst pipeline), PRD-28 (ndxplorer)
...
histogram width is shot noise, static heterogeneity, or dynamics. PAM's `PDAFit` and
`tcPDA` are mature; ChiSurf has the compute engine but no analysis surface.
...
unavailable at the application level (PRD-49 status: ENGINE-ONLY). This is the highest-
reuse gap: the math exists; we need the model+UI+fit integration.
...
- Full time-binned dynamic PDA (N-vs-observation-time grid like PAM's
  `NumberOfBinsE`/`NumberOfBinsT`) and a 3-state variant — current model uses a
...
5. **Kinetic consistency check.** Resample burst data from a fitted kinetic scheme and
   compare to the measured histogram (PAM's "consistency check"); reuses the dynamic-PDA
   simulator.
...
- **UI = AutoForm + `view.json`** (PRD-49 §2 mandate): parameters (distances, widths,
  populations, corrections, kinetic rates) via `parameter_table`; histogram/fit overlay
  via an AutoForm plot section. No bespoke Qt. Add a section to
...
- PRD-49 §4 structural-FRET stack — P(R) priors can come from AV models (`fps_json_editor`,
  `fret_line`), a differentiator over PAM.
...
- No new bespoke Qt widgets (PRD-49 §2).
