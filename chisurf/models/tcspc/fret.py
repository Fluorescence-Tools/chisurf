...
class Gaussians(FittingParameterGroup):
    ...
    def distribution(self) -> np.array:
        ...
        if not self.is_distance_between_gaussians:
            ...
            pdf = chisurf.math.functions.distributions.generalized_normal_distribution
        else:
...
class WormLikeChainModel(FRETModel):
    ...
    def distance_distribution(self):
        ...
        if not self.use_dye_linker:
            prob = chisurf.math.functions.rdf.worm_like_chain(
                rda_axis,
                kappa,
                chain_length
            )
            ...
            prob = chisurf.math.functions.rdf.worm_like_chain_linker(
                rda_axis, kappa,
                chain_length,
                sigma_linker
            )
