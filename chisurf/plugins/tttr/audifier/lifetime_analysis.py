...
def plot_lifetime_waterfall(
    ...
    *,
    ...
    im = plt.imshow(plot_data, aspect='auto', extent=extent, 
...
def plot_lifetime_waterfall_multichannel(
    ...
    *,
    ...
    for i, ch in enumerate(channels):
        ...
        extent = [tau[0], tau[-1], macro_t_s[-1], macro_t_s[0]]
        im = ax.imshow(plot_data, aspect='auto', extent=extent,
                      cmap='viridis', origin='upper', vmin=0, vmax=vmax)
        
        ax.set_ylabel(f"Channel {ch}\nMacro time (s)")
        ax.set_xscale('log')
        
        # Add channel label
        ax.text(0.02, 0.98, f"Ch {ch}", transform=ax.transAxes,
