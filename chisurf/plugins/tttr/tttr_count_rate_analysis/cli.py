...
def calculate_count_rates(
    ...
    for file_idx, file_path in enumerate(tttr_files):
        if verbose:
            click.echo(f"Processing file {file_idx+1}/{len(tttr_files)}: {os.path.basename(file_path)}")
        ...
        try:
            ...
            if verbose:
                click.echo(f"  Processed {os.path.basename(file_path)}: {measurement_time:.2f}s, {sum(file_n_photons.values())} photons")
...
def save_results_as_txt(
    ...
    try:
        with open(output_file, 'w') as f:
            ...
            # Write header
            headers = ["Channel", "Mean Count Rate (kHz)", "Std Count Rate (kHz)", "#Photons", "Measurement Time (s)"]
...
@click.option('--setup-file', '-s', type=click.Path(exists=True), 
...
def analyze(files, setup_file, setup_name, output, verbose):
    ...
    if not setup_name:
        ...
        if setup_idx < 1 or setup_idx > len(available_setups):
            click.echo(f"Invalid selection. Please choose a number between 1 and {len(available_setups)}", err=True)
            return
