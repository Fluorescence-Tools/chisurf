...
This script provides a command-line interface (CLI) for:
  ...
  - Aggregating all molecule properties into a combined TSV file (only if >1 PTU contributed).
"""
...
def compute_irf_component(
    ...
    This function filters photon events by micro-time range and routing channel,
    then computes a histogram with the given binning.
    """
    ...
    # Build boolean mask for events in desired micro-time & channels
    mask = (
            (mt_arr >= raw_start) &
            (mt_arr <= raw_stop) &
    ...
    # Compute histogram at the requested binning
    hist, _ = sub.get_microtime_histogram(micro_time_binning, minlength=minlength)

    # Return only the bins in [start_bin, stop_bin)
...
def load_irf(
    ...
    """
    # Load the TTTR object for the IRF
    irf_tttr = tttrlib.TTTR(irf_file)

    # Split channels into parallel (sp) and perpendicular (ss)
    ...
    # Normalize each channel to sum=1 (if non-zero)
...
def filter_tttr(
    ...
    """
    Filter a TTTR object by micro-time range and routing channels, returning a new TTTR object.
    """
    start_bin, stop_bin = micro_time_range
    raw_start = start_bin * micro_time_binning
    raw_stop = stop_bin * micro_time_binning
...
def make_jordi(
    ...
    from a subset TTTR object representing a single molecule.
    """
    if len(detector_chs) >= 2:
...
def process_ptu_file(
    ...
    and save outputs (images, indices, Jordi files, fit plots). Returns a DataFrame
    summarizing properties of all detected molecules.
    """
    fn = str(ptu_path)
    ...
    # Load PTU and check if it contains CLSM image data
    d = tttrlib.TTTR(fn)
    import json
    ...
    # Build combined intensity image (sum over all detector channels)
    ...
    # Save each channel's intensity image separately
    ...
    # Compute micro-time resolution and effective period (ns)
    dt_micro = d.header.micro_time_resolution  # seconds per micro-time bin
    dt_micro_ns = dt_micro * 1e9  # convert to ns
    ...
    coords = peak_local_max(distance, footprint=np.ones((6, 6)), labels=clean)
    ...
    # Normalize smoothed image to uint8 for overlays
    ...
    for prop in measure.regionprops(labels):
        ...
        # Collect all TTTR indices from pixels belonging to this molecule
        tttr_list = []
        for (r, c_) in coords:
        ...
        # Save TTTR indices for this molecule
        ...
        # Compute Jordi vector for this molecule
        ...
        # Fit the Jordi vector using Fit23 (biexponential with IRF + background)
        fit = tttrlib.Fit23(
            dt=DT_EFFECTIVE_ns,
            irf=irf_full,
            background=raw_irf,
        ...
        # Generate log-scale plot: Jordi, Fit, and scaled IRF
        plt.figure()
        ...
        # Compute shape metrics for this region
        ...
        # Create a color overlay image: molecules as red, background grayscale
        ...
        mask = (labels == lab)
        rgb[mask, 0] = smoothed_uint8[mask]
        rgb[mask, 1] = 0
        rgb[mask, 2] = 0
    ...
    # Build DataFrame of all molecules in this PTU
...
def cli():
    ...
    Use the 'run' command to process PTU files. The DYMGroup provides
    suggestions for mistyped commands or options.
    """
    pass
...
@click.option(
    ...
    help='Path to the PTU file used for Instrument Response Function (IRF) measurement.'
)
@click.option(
    '--detector-chs', '-d',
    default=[2, 0],
    ...
    type=int,
    multiple=True,
    help='List of detector channels: even indices → parallel, odd → perpendicular (e.g., -d 2 0).'
)
@click.option(
    '--micro-time-range', '-r',
    default=(0, 256),
    ...
    type=int,
    nargs=2,
    help='Inclusive micro-time bin range (start stop) after binning (e.g., -r 0 256).'
)
@click.option(
...
@click.option(
    ...
    help=(
            ...
            '  0 = no normalization\n'
            '  1 = normalize by average rate across both channels\n'
            '  2 = normalize each channel separately to unit area\n'
            '  3 = normalize by total acquisition time'
    )
)
@click.option(
    '--threshold', '-t',
    ...
    show_default=True,
    type=float,
    help='Threshold fraction (0 < t < 1) to zero out low-intensity bins in Jordi histograms.'
)
@click.option(
    '--minlength',
    default=-1,
    show_default=True,
    type=int,
    help='Minimum histogram length for Jordi vectors; use -1 for auto.'
)
@click.option(
    '--shift-sp',
    default=0.0,
    show_default=True,
    type=float,
    help='Fractional shift to apply to the parallel IRF component (in bins).'
)
@click.option(
    '--shift-ss',
    default=0.0,
    show_default=True,
    type=float,
    help='Fractional shift to apply to the perpendicular IRF component (in bins).'
)
@click.option(
    ...
    type=float,
    help=(
            'Zero out any IRF bin < (irf-threshold-fraction * IRF_max) '
            '(e.g., 0.08).'
    )
)
@click.option(
    ...
    nargs=4,
    help=(
            'Initial guess for Fit23 decay parameters: '
            '(tau0, gamma0, rho0, scale0).'
    )
)
@click.option(
    '--fit-fixed-flags',
    ...
    nargs=4,
    help=(
            'Flags to fix Fit23 parameters: '
            '(tau_fixed, gamma_fixed, rho_fixed, scale_fixed) '
            '0 = free, 1 = fixed.'
    )
)
@click.option(
...
@click.option(
    ...
    default=True,
    show_default=True,
    help='Enable (--twoi-star) or disable (--no-twoi-star) the twoI* flag in Fit23.'
)
@click.option(
    '--bifl-scatter/--no-bifl-scatter',
    default=False,
    show_default=True,
    help='Enable (--bifl-scatter) or disable (--no-bifl-scatter) the bifurcation scatter flag in Fit23.'
)
@click.option(
    '--output-dir', '-o',
    default='.',
    ...
    help='Directory in which to search for PTU files and save output subfolders.'
)
@click.option(
    '--output-file', '-f',
    ...
    help=(
        'Path (relative or absolute) where the combined TSV of all molecules '
        'will be written. If unset, defaults to "./combined_molecule_data.tsv".'
    )
)
def run(
        ptu_pattern: str,
    ...
    This command will:
      1. Change working directory to --output-dir.
      2. Load and normalize the IRF from --irf-file.
      3. Glob all PTU files matching --ptu-pattern under --output-dir.
      4. For each PTU file:
         a. Segment microscopy image to find molecules.
         b. Extract photon indices for each molecule’s pixels.
         c. Compute Jordi vectors and fit decay curves (Fit23).
         d. Save output:
            - Total intensity TIFFs
            - Per-channel TIFFs
            - Molecule-specific TTTR indices
            - Jordi vector files (.jordi)
            - Fit plots (.png)
            - Segmentation overlays (.png)
            - A per-PTU TSV and pickle of molecule table
      5. If more than one PTU produced molecules, concatenate per-PTU tables into the specified combined TSV.
         If exactly one PTU produced molecules, skip creating a combined TSV.
    """
    # Convert tuples to the appropriate types
    ...
    # Validate and enter output directory
    ...
    # Combine per-PTU tables only if more than one contributed
    if len(all_dfs) > 1:
        ...
        click.echo(f"Combined molecule data saved to: {combined_path}")
    elif len(all_dfs) == 1:
        click.echo(
            "Only one PTU produced molecule data. "
            "Skipping creation of a combined TSV (per-file TSV is already in its own folder)."
        )
    else:
