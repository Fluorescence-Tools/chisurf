...
class StateControlDock(QtCore.QObject):
    """PyMOL-inspired control panel (content widget, no outer QDockWidget)."""
    ...
    def __init__(
        ...
        *,
        ...
        self.lbl_mouse = ClickableLabel(self._widget)
        ...
        self.lbl_selection = QtWidgets.QLabel(self._widget)
        self.lbl_selection.setTextFormat(QtCore.Qt.RichText)
        layout.addWidget(self.lbl_selection)

        # 3. State display
        self.lbl_state = QtWidgets.QLabel(self._widget)
        ...
        self.btn_zoom = QtWidgets.QToolButton(self._widget)
        self.btn_zoom.setText("Zoom")
        self.btn_zoom.setStyleSheet(
            "padding: 2px 8px; background-color: #333; border: 1px solid #555;"
        )
        self.btn_zoom.clicked.connect(lambda: self.cmd.do("zoom"))

        self.btn_orient = QtWidgets.QToolButton(self._widget)
        ...
        def _make_play_btn(text: str, cmd_str: str, tip: str):
            ...
            btn.setStyleSheet(
                "background-color: #222; border: 1px solid #444; color: #f88;"
            )
        ...
        play_layout.addWidget(
            _make_play_btn("<", "frame -1", "Previous frame")
        )
        play_layout.addWidget(_make_play_btn(" \u25a0 ", "mstop", "Stop"))
        play_layout.addWidget(_make_play_btn(" \u25b6 ", "mplay", "Play"))
        play_layout.addWidget(
            _make_play_btn(" > ", "frame +1", "Next frame")
        )
        play_layout.addWidget(
            _make_play_btn(">|", "frame last", "Go to last frame")
        )
    ...
    def refresh_ui(self) -> None:
        ...
        self.lbl_selection.setText(
            f"<span style='color: #00FF00;'>Selecting</span> "
            f"<span style='color: #00FFFF;'>{sel_mode}</span>"
        )

        self.lbl_state.setText(
            f"<span style='color: #00FF00;'>State</span>&nbsp;&nbsp;&nbsp;&nbsp;"
            f"<span style='color: #FFF;'>{curr} / {total}</span>"
        )
