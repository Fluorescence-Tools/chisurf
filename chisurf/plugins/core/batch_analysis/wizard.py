...
class FileAndFitSelectionPage(QWizardPage):
    ...
    def populate_fit_combo_box(self):
        """Populate the fit combo box from ``cs.fits`` and try to restore the previous selection."""
        try:
    ...
    def get_selected_fit_index(self) -> int:
        """Return the index of the selected fit in cs.fits using object identity when possible."""
        try:
...
class AnalysisPage(QWizardPage):
    ...
    def dummy_run_fit(self, file, fit_idx):
        ...
        This function calls cs to load the file and update the chosen fit's data.
        """
