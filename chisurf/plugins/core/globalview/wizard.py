...
class GraphWizard(QtWidgets.QWidget):
    ...
    def build_graph(
            ...
            fit_list: list[cs.core.fitting.fit.FitGroup] = None,
    ...
    def make_graph(
        ...
        for node in G.nodes:
            o = node_objects[node]
            if isinstance(o, cs.core.parameter.Parameter):
                if o.fixed:
                    if not include_fixed:
                        continue
                    else:
    ...
    def make_graph_plot(
            self,
            fit_list: list[cs.core.fitting.fit.FitGroup],
