...
def propagate_node(node_type, config, input_spectra, db):
    ...
    if node_type == "light_source":
        ...
        if mode == "database":
            ...
            if probe_id:
                with db:
                    s = db.get_probe_spectrum(probe_id, "emission")
                    y = interp(s)
            else:
                y = np.zeros_like(WAVELENGTHS)
        ...
        probe_ids = config.get("probe_ids", config.get("spectrum_ids", []))
        if not probe_ids and config.get("probe_id", config.get("spectrum_id")):
            probe_ids = [config.get("probe_id", config.get("spectrum_id"))]
        ...
        if probe_ids:
            ...
            with db:
                ...
                for probe_id in probe_ids:
                    abs_s_raw = db.get_probe_spectrum(probe_id, "absorption")
                    em_s_raw = db.get_probe_spectrum(probe_id, "emission")
                    props = db.get_standardized_optical_properties(probe_id)
                    item_data = db.get_probe_by_id(probe_id)
                    dye_name = item_data['chromophore_name'] if item_data else f"Probe_{probe_id}"
                    ...
                    if probe_id_str in dye_props:
                        try:
                            qy = float(dye_props[probe_id_str].get("qy", qy_str))
                            ec = float(dye_props[probe_id_str].get("ec", ec_str))
        ...
        if probe_id:
            with db:
                s = db.get_probe_spectrum(probe_id, "quantum_efficiency")
                qe_y = interp(s)
                node_char = qe_y
                out_dict = {}
                for src_id, in_spec in spectral_in.items():
