"""Self-contained sound manager for Tetris game.
...
def _generate_sweep_wav(
    ...
    """Write a frequency-sweep WAV file."""
    n_samples = int(sample_rate * duration)
    samples = []
    for i in range(n_samples):
        t = i / sample_rate
...
class SoundManager:
    ...
    def ensure_sounds(self) -> None:
        """Generate all WAV files on first call (lazy initialisation)."""
        if self._tmpdir is not None:
            return
        self._tmpdir = Path(tempfile.mkdtemp(prefix="tetris_sounds_"))
        self._generate("rotate", _generate_wav, 523, 0.08, volume=0.2)
        self._generate("drop", _generate_wav, 262, 0.15, volume=0.25)
        self._generate("line_clear", _generate_sweep_wav, 660, 1320, 0.3, volume=0.3)
        self._generate("game_over", _generate_wav, 196, 0.6, volume=0.3, fade_out=True)
