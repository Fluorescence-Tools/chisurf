...
def download_fpbase_to_db(db):
    """Downloads FPbase summaries and spectra for items that HAVE spectra.
    ...
    with db:
        ...
        # Get existing FPbase slugs in database to identify ones to remove
        db.cursor.execute("SELECT probe_id, property_value FROM optical_properties WHERE property_name = 'fpbase_slug'")
        existing_items = {row[1]: row[0] for row in db.cursor.fetchall()}
        ...
        # Cleanup removed to prevent accidental deletion of curated/legacy data.
        # for existing_slug, db_item_id in existing_items.items():
        #     if existing_slug not in slugs_with_spectra:
        #         print(f"Skipping removal of item without spectra: {existing_slug}")
