...
class BurbulatorSimulator:
    ...
    def _find_burbulator_dll(self):
        """Find the Burbulator DLL."""
        import sys
        is_64bit = sys.maxsize > 2**32

        # Look in the same directory as this module first
        module_dir = os.path.dirname(os.path.abspath(__file__))

        # Try 64-bit DLL first if we're on 64-bit Python
        dll_names = ['burbulator_x64.dll', 'burbulator.dll'] if is_64bit else ['burbulator.dll', 'burbulator_x64.dll']

        for dll_name in dll_names:
            dll_path = os.path.join(module_dir, dll_name)
            if os.path.exists(dll_path):
                logger.info("SIMULATION: Found DLL at %s", dll_path)
                return dll_path

        # Look in the original Burbulator directory
        burbulator_dir = r"E:/dev/tttrlib/playground/Burbulator"
        for dll_name in dll_names:
            dll_path = os.path.join(burbulator_dir, dll_name)
            if os.path.exists(dll_path):
                logger.info("SIMULATION: Found DLL at %s", dll_path)
                return dll_path
    ...
    def _setup_function_prototypes(self):
        ...
        self.dll.smdif_ov3.argtypes = [
            ...
            ct.POINTER(ct.c_ulong),   # T0
            ct.POINTER(ct.c_int),     # N_molecules
        ...
        self.dll.smdif_ov3.restype = ct.c_ulong

        # data2spc132_tac function signature
