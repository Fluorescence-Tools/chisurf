    ...
    if (info not in [1, 2, 3, 4] and not full_output):
        if info in [5, 6, 7, 8]:
            ...
            try:
                ...
                raise errors[0][1](errors[0][0])

    popt = i2e(retval[0])

    if full_output:
        # convert fjac from internal to external parameters
        grad = _internal2external_grad(retval[0], bounds)
        # Check if retval[1] is a matrix or a dictionary
        if isinstance(retval[1], dict):
            # If it's a dictionary, just pass it through
            cov_x = retval[1]
        else:
        ...
        if len(retval) >= 5:
            return popt, cov_x, retval[2], retval[3], retval[4]
        elif len(retval) == 4:
            return popt, cov_x, retval[2], retval[3], None
        elif len(retval) == 3:
            return popt, cov_x, retval[2], None, None
        else:
            # Provide default values for missing elements
            infodict = retval[2] if len(retval) > 2 else {}
            mesg = "Unknown error" if len(retval) <= 3 else retval[3]
            ier = 0 if len(retval) <= 4 else retval[4]
            return popt, cov_x, infodict, mesg, ier
    else:
        return popt
