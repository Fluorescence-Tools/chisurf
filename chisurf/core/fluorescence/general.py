...
def species_averaged_lifetime(
    ...
    if is_lifetime_spectrum:
        x, t = chisurf.core.math.datatools.interleaved_to_two_columns(fluorescence)
        if normalize:
            xs = x.sum()
            if xs > 0:
                x /= xs
...
def calculate_fluorescence_decay(
    ...
    >>> import chisurf.core.structure.structure  # doctest: +SKIP
    >>> time_axis = np.linspace(0, 20, num=100)  # doctest: +SKIP
    >>> structure = chisurf.core.structure.structure.Structure('./test/data/modelling/pdb_files/hGBP1_closed.pdb')  # doctest: +SKIP
...
def compute_mean_fret(
    ...
    da_amplitudes, da_lifetimes = chisurf.core.math.datatools.interleaved_to_two_columns(
        lifetime_spectrum
    )
    d0_amplitudes, d0_lifetimes = chisurf.core.math.datatools.interleaved_to_two_columns(
