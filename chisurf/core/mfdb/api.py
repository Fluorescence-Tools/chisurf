...
def _check_acl_filter(conn: Any, principal: Any, object_type: str, rows: list, id_key: str) -> list:
    ...
    for row in rows:
        ...
        if r:
            ...
            break
    if has_acls:
        from chisurf.core.mfdb.auth import filter_readable
...
def graph_upstream(
    ...
    """
    with MFDatabase(resolve_database_path()) as db:
        from chisurf.core.mfdb.graph import traverse_canonical_graph as traverse
        edges = traverse(
            db.conn,
            node_type,
            node_id,
...
def save_chinet_session(
    ...
    """
    from chisurf.core.mfdb.chinet_adapter import store_chinet_session
...
def get_chinet_session(artifact_id: str) -> dict[str, Any]:
    ...
    """
    from chisurf.core.mfdb.chinet_adapter import load_chinet_session
