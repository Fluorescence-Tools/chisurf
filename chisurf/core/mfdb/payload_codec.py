...
def _validate_cross_field_shapes(payload: Any) -> None:
    if isinstance(payload, GenericCurve):
    ...
    if isinstance(payload, Spectrum) and payload.intensity.shape != payload.wavelength.shape:
        raise PayloadSchemaError("spectra.intensity must match wavelength shape")
    if getattr(payload, "KIND", "") == "tcspc_decay":
