...
def read_tcspc_csv(
    ...
    csvSetup = chisurf.core.fio.ascii.Csv(
        *args,
        **kwargs
    )
    csvSetup.load(
    ...
    if is_jordi:
        ...
        if polarization == 'vv':
            ...
            ey = chisurf.core.fluorescence.tcspc.counting_noise(decay=y)
            ...
            # Calculate errors
            e1 = chisurf.core.fluorescence.tcspc.counting_noise(decay=vv)
            e2 = chisurf.core.fluorescence.tcspc.counting_noise(decay=vh)
            ey = np.vstack([e1, e2])
