...
from chisurf.core.structure.av import DynamicAV
...
class AVDecayModel(LifetimeModel):
    ...
    def __init__(
        ...
        Example
        -------
        >>> import chisurf.core.curve  # doctest: +SKIP
        >>> import chisurf.core.fitting  # doctest: +SKIP
        >>> data_set = chisurf.core.data.DataCurve(filename='./test/data/tcspc/ibh_sample/Decay_577D.txt', skiprows=9)  # doctest: +SKIP
        >>> lin = chisurf.core.data.DataCurve(filename='./test/data/tcspc/ibh_sample/whitelight.txt', skiprows=9)  # doctest: +SKIP
        >>> data_set.ey = chisurf.core.fluorescence.tcspc.counting_noise(data_set.y)  # doctest: +SKIP
        >>> irf = chisurf.core.data.DataCurve(filename='./test/data/tcspc/ibh_sample/Prompt.txt', skiprows=9)  # doctest: +SKIP
        >>> data_set.x *= 0.0141  # doctest: +SKIP
        >>> irf.x *= 0.0141  # doctest: +SKIP
        >>> data_set = chisurf.core.curve.ExperimentDataCurveGroup(data_set)  # doctest: +SKIP
        >>> structure = chisurf.core.structure.Structure('./test/data/atomic_coordinates/pdb_files/hGBP1_closed.pdb')  # doctest: +SKIP
        >>> from chisurf.core.fitting.model.tcspc.av_decay import AVDecayModel  # doctest: +SKIP
