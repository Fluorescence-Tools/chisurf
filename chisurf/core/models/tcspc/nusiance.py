...
class Generic(FittingParameterGroup):
    ...
    @property
    def background_curve(self) -> chisurf.core.curve.Curve:
    ...
    def __init__(
            self,
            background_curve: chisurf.core.data.DataCurve = None,
            name: str = 'Nuisance',
            **kwargs
    ):
...
class Corrections(FittingParameterGroup):
    ...
    def calc_lintable(
        ...
        return chisurf.core.fluorescence.tcspc.corrections.compute_linearization_table(
            y,
            window_length,
            window_function,
            xmin,
    ...
    def pileup(self, decay: np.array, **kwargs):
        ...
        if self.correct_pile_up:
            chisurf.core.fluorescence.tcspc.corrections.add_pile_up_to_model(
                data,
                decay,
                rep_rate,
                dead_time,
...
class Convolve(FittingParameterGroup):
    ...
    def _process_irf(self, normalize: bool = True) -> chisurf.core.curve.Curve:
        ...
        if irf_start_idx > 0 or irf_stop_idx < len(irf.y):
            ...
            irf = chisurf.core.curve.Curve(x=irf.x, y=irf_y)
    ...
    def _irf(self) -> chisurf.core.curve.Curve:
        ...
        #     re = chisurf.core.curve.Curve(x, y)
        return re

    @_irf.setter
    def _irf(self, v: chisurf.core.curve.Curve):
        """Stored IRF curve (private)."""
    ...
    def scale(
            self,
            decay: chisurf.core.data.DataCurve,
            start: int = None,
            stop: int = None,
            bg: float = 0.0,
        ...
        if autoscale:
            weights = 1.0 / data.ey
            n0 = chisurf.core.fluorescence.tcspc.rescale_w_bg(
                model_decay=decay,
    ...
    def convolve(
        ...
        if mode == "per":
            ...
            # time = fit.data._x
            # chisurf.core.fluorescence.tcspc.fconv_per_dt(model_decay, lifetime_spectrum, irf_y, start, stop, n_points, period,
            # time)
        elif mode == "exp":
            t = self.data.x
    ...
    def __init__(
        ...
        self._n0 = FittingParameter(
            value=chisurf.core.settings.cs_settings['tcspc']['n0'],
            name='n0',
        ...
        self.mode = chisurf.core.settings.cs_settings['tcspc']['default_convolution_mode']
    ...
    def get_state(self) -> dict:
        ...
        try:
            ...
            if isinstance(irf, chisurf.core.curve.Curve):
        ...
        if isinstance(irf_state, dict):
            try:
                ...
                if x.size and x.size == y.size:
                    curve = chisurf.core.curve.Curve(x=x, y=y)
