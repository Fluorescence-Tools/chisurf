...
class LifetimeMixtureModel(LifetimeModel):
    ...
    @fractions.setter
    def fractions(self, vs) -> None:
        """Set raw (unnormalized) mixing weights; normalized on read."""
        for fp, v in zip(self._fractions, vs):
            fp.value = max(float(v), 1e-9)
...
class LifetimeMixtureNewModel(LifetimeMixtureModel):
    """Lifetime mixer whose editor is auto-generated by the AutoForm framework (PRD-40).

    Identical computation to :class:`LifetimeMixtureModel`, registered as a
    *pure* (Qt-free) model so the GUI renders its editor via ``AutoModelWidget``
    from ``mix_model.view.json``. The custom ``fit_mixer`` section in that JSON
    drives the fit-selection, add/remove controls and fraction parameters.

    Registered **additively** as "Lifetime mixer (new)" alongside the proven
    hand-written ``LifetimeMixtureModelWidget`` so the AutoForm path can be
    exercised live without replacing the existing model.
    """

    name = "Lifetime mixer (new)"
    view_spec_file = "mix_model.view.json"
