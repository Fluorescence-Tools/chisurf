...
def av_distance_distribution(
    ...
    >>> structure = chisurf.core.structure.Structure('./test/data/modelling/pdb_files/hGBP1_closed.pdb')
    ...
    """
    av_donor = chisurf.core.structure.av.ACV(structure, **donor_av_parameter)
    av_acceptor = chisurf.core.structure.av.ACV(structure, **acceptor_av_parameter)
...
def av_filtered_fcs_weights(
    ...
    >>> from chisurf.core.fluorescence.fcs.filtered import calc_lifetime_filter
...
class LabeledStructure(Structure):
    ...
    donor_label : dict
        A dictionary which describes the labeling position of the donor (see :py:class:`chisurf.core.structure.av.AV`)

    acceptor_label : dict
        A dictionary which describes the labeling position of the acceptor (see :py:class:`chisurf.core.structure.av.AV`)
    ...
    def acceptor_label(self, v):
        ...
        self._acceptor_av = chisurf.core.structure.av.ACV(self, **self._donor_description)
    ...
    def lifetime_spectrum(self) -> np.array:
        ...
        return chisurf.core.fluorescence.general.rates2lifetimes(rs, ds)
