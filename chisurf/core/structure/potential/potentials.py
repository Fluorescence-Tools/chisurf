...
class CEPotential(object):
    ...
    >>> s = chisurf.core.structure.Structure('./test/data/model/hgbp1/hGBP1_closed.pdb', verbose=True, make_coarse=True)
    >>> pce = chisurf.core.structure.potential.potentials.CEPotential(s, ca_cutoff=64.0)
    >>> pce.getEnergy()
    -0.15896629131635745
    """
...
class ClashPotential(object):
    ...
    def __init__(
        ...
        >>> import chisurf.core.structure.potential
        >>> s = chisurf.core.structure.Structure('./test/data/model/hgbp1/hGBP1_closed.pdb', verbose=True, make_coarse=True)
        >>> pce = chisurf.core.structure.potential.ClashPotential(structure=s, clash_tolerance=6.0)
        >>> pce.getEnergy()
...
#     >>> import chisurf
#     >>> av_potential = chisurf.core.structure.potential.potentials.AvPotential(distances=distances, positions=positions)
#     >>> structure = chisurf.core.structure.Structure('./test/data/model/HM_1FN5_Naming.pdb')
...
#             av_samples = chisurf.core.settings["fps"]["distance_samples"]
...
#         ]
#         avs = map(
#             lambda x: chisurf.core.structure.av.BasicAV(**x),
#             arguments
#         )
...
#     ):
#         if isinstance(structure, chisurf.core.structure.Structure):
#             self.structure = structure
#         if gauss_bond:
#             energy = 0.0
#             self.calc_distances()
