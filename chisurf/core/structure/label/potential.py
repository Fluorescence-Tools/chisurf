...
import chisurf.core.structure.label
import chisurf.core.structure.label.distribution
...
import chisurf.core.structure.label.av
...
class AvPotential(object):
    ...
    >>> import chisurf.core.structure.label.av
    >>> import scikit_fluorescence.io.structure
    >>> av_potential = chisurf.core.structure.label.av.AvPotential(distances=distances, positions=positions)
    ...
    >>> av_potential = chisurf.core.structure.label.av.AvPotential(labeling_file=labeling_file, atoms=atoms)
    ...
    def score_structure(
        ...
        >>> chi2 = chisurf.core.structure.label.av.AvPotential.score_structure(imp_hier, labeling_file)
        >>> chi2
        1.125509321839683
    ...
    def calc_distances(
        ...
        for distance_key in self.distances:
            ...
            if distance_type == 'RDAMean':
                d12 = chisurf.core.structure.label.functions.average_distance(av2)
            elif distance_type == 'Rmp':
                d12 = chisurf.core.structure.label.functions.distance_between_mean_positions(av1, av2)
            elif distance_type == 'RDAMeanE':
                d12 = av1.RDAE(av1, av2, R0)
            elif distance_type == 'get_distance_distribution':
               rda = np.array(distance['rda'])
               d12 = chisurf.core.structure.label.functions.histogram_rda(
                   dd1=av1,
                   dd2=av2,
                   rda_axis=rda
               )[0]
