...
def to_elementary(
    ...
    skip_qt_widgets : bool
        If set to True (the default value is False) Qt widgets will be skipped during conversion to avoid serialization issues.
...
class Base(object):
    ...
    def to_dict(
        ...
        convert_values_to_elementary: bool
            ...
            types.
        skip_qt_widgets: bool
            If this parameter is set to True (default False), Qt widgets will be skipped during dictionary creation to avoid serialization issues.
...
def set_safe_import_notify(callback):
    """Set a callback ``(title, text) -> None`` for missing-package alerts."""
    global _safe_import_notify
    _safe_import_notify = callback
