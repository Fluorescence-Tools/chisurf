...
class GenericCurve:
    ...
    @classmethod
    def from_curve_like(cls, curve: Any) -> GenericCurve:
        """Create a payload from an object exposing curve arrays.

        Parameters
        ----------
        curve : Any
            Source object exposing ``x`` and ``y`` arrays, plus optional ``ex``,
            ``ey``, and ``mask`` arrays.
        ...
        """
        return cls(
            x=np.asarray(curve.x, dtype=np.float64).copy(),
            y=np.asarray(curve.y, dtype=np.float64).copy(),
            ex=(
                np.asarray(curve.ex, dtype=np.float64).copy()
                if getattr(curve, "ex", None) is not None
                else None
            ),
            ey=(
                np.asarray(curve.ey, dtype=np.float64).copy()
                if getattr(curve, "ey", None) is not None
                else None
            ),
            mask=(
                np.asarray(curve.mask, dtype=bool).copy()
                if getattr(curve, "mask", None) is not None
                else None
            ),
        )

    from_data_curve = from_curve_like
    ...
    def to_curve_kwargs(self) -> dict[str, np.ndarray | None]:
        """Return copied arrays suitable for constructing a curve object.

        Returns
        -------
        dict
            Keyword arguments containing copied ``x``, ``y``, ``ex``, ``ey``,
            and ``mask`` arrays.
        """
        return {
            "x": self.x.copy(),
