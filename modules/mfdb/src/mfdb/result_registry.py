...
def register_result(
    ...
    data : str, bytes, Path, pandas.DataFrame, dict, or None, optional
        ...
        stored directly; dictionaries, data frames, curve-like objects, and
...
def _coerce_payload(kind: str, data: Any, pandas_module: Any = None) -> tuple[str, Any]:
    ...
    if _looks_like_data_curve(data):
        return GenericCurve.KIND, GenericCurve.from_curve_like(data)
...
def _looks_like_data_curve(data: Any) -> bool:
    """Return whether an object has the curve attributes needed for archival.
