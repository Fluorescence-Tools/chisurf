"""Curated seed data for the MFDB fluorescence database."""
...
def _overlap_integral(
    ...
    """Compute the spectral overlap integral for Förster-radius seeding.

    Parameters
    ----------
    wavelength_nm : np.ndarray
        One-dimensional wavelength grid in nanometers.
    donor_emission : np.ndarray
        Donor emission intensity on ``wavelength_nm``.
    acceptor_extinction : np.ndarray
        Acceptor molar extinction coefficient on ``wavelength_nm``.

    Returns
    -------
    float
        Overlap integral in M^-1 cm^-1 nm^4.
    """
    wl = np.asarray(wavelength_nm, dtype=float)
    donor = np.asarray(donor_emission, dtype=float)
    acceptor = np.asarray(acceptor_extinction, dtype=float)
    if not (wl.shape == donor.shape == acceptor.shape) or wl.ndim != 1 or wl.size < 2:
        raise ValueError("spectra must be one-dimensional arrays with matching shape")
    area = np.trapz(donor, wl)
    if area <= 0:
        raise ValueError("donor emission spectrum has non-positive area")
    return float(np.trapz((donor / area) * acceptor * wl**4, wl))


def _forster_radius_angstrom(
    ...
    *,
    ...
    """Compute Förster radius in angstrom from a spectral overlap integral.
    ...
    if overlap_integral < 0:
        raise ValueError("overlap integral must be non-negative")
    if donor_quantum_yield < 0:
        raise ValueError("donor quantum yield must be non-negative")
    if refractive_index <= 0:
        raise ValueError("refractive index must be positive")
    r0_nm = 0.02108 * (
        kappa_squared * donor_quantum_yield * refractive_index**-4 * overlap_integral
    ) ** (1.0 / 6.0)
    return float(r0_nm * 10.0)
...
def _seed_forster_radii(db: MFDatabase) -> None:
    ...
    for donor_name, acceptor_name in _COMMON_FRET_PAIRS:
        ...
        try:
            ...
            J = _overlap_integral(common_wl, fd, ea)
            R0 = _forster_radius_angstrom(J, donor_quantum_yield=donor_qy)
        except Exception:
            continue
...
def _seed_entities(db: MFDatabase) -> None:
    db.add_entity(
        ...
        "T4 lysozyme",
        entity_type="polymer",
        details="T4 lysozyme curated FRET sample",
    )
    ...
    db.add_entity(
        ...
        "HIV RT DNA",
        entity_type="polymer",
        details="HIV reverse transcriptase DNA duplex seed",
    )
...
def _seed_analyses(db: MFDatabase) -> None:
    ...
    sp1 = db.add_sample_probe(
        "148L_65_141_A488_A594",
        _probe_id(db, "Alexa488"),
        fluorophore_type="donor",
        description="Donor at 65",
        poly_probe_position_id=a488_pos,
    )
    sp2 = db.add_sample_probe(
        "148L_65_141_A488_A594",
        _probe_id(db, "Alexa594"),
        fluorophore_type="acceptor",
        description="Acceptor at 141",
        poly_probe_position_id=a594_pos,
    )
    db.add_sample_probe(
        "148L_intrinsic_trp",
        _probe_id(db, "Trp"),
        fluorophore_type="donor",
        description="Intrinsic Trp",
        poly_probe_position_id=_position_id(db, "Trp", "148L", 126),
    )
    ...
    sp3 = db.add_sample_probe(
        ...
        _probe_id(db, "Cy3"),
        fluorophore_type="donor",
        description="DNA donor",
        poly_probe_position_id=cy3_pos,
    )
    sp4 = db.add_sample_probe(
        "1RTD_DNA_10_20_Cy3_Cy5",
        _probe_id(db, "Cy5"),
        fluorophore_type="acceptor",
        description="DNA acceptor",
        poly_probe_position_id=cy5_pos,
    )
    db.add_sample_probe(
        "1RTD_DNA_10_20_Cy3_Cy5",
        _probe_id(db, "2-aminopurine"),
        fluorophore_type="donor",
        description="2AP reference",
        poly_probe_position_id=_position_id(db, "2-aminopurine", "1RTD_DNA", 12),
    )
