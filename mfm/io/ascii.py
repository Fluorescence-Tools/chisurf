...
class Csv(object):
    ...
    def load(self, filename, **kwargs):
        ...
        guess_dialect = kwargs.get('guess_dialect', True)
        ...
        if os.path.isfile(filename):
            ...
            if guess_dialect:
                ...
                dialect = sniffer.sniff(txt[-10])
            else:
                dialect = None

            if self.mode == 'csv':
                df = pd.read_csv(filename, skiprows=skiprows, dialect=dialect, header=header)
            else:
                df = pd.read_fwf(filename, colspecs=colspecs, skiprows=skiprows, header=header)
            self.data = df

    def save(self, data, filename, delimiter='\t', mode='txt', header='', ending=None):
        if self.verbose:
            print "Saving"
            print "------"
