...
from mfm.parameter import FittingParameter, AggregatedParameters
...
class ParseFormula(AggregatedParameters):
    ...
    def __init__(self, **kwargs):
        ...
        self.model_file = kwargs.get('model_file', os.path.join(mfm.package_directory, 'settings/fcs.model.json'))
    ...
    def load_model_file(self, filename):
        with open(filename, 'r') as fp:
            ...
            self.models = json.load(fp, object_pairs_hook=OrderedDict)
...
class ParseFormulaWidget(ParseFormula, QtGui.QWidget):

    def __init__(self, **kwargs):
        QtGui.QWidget.__init__(self)
        uic.loadUi('mfm/ui/models/parseWidget.ui', self)
        ParseFormula.__init__(self, **kwargs)
        self.n_columns = kwargs.get('n_columns', mfm.settings['gui']['fit_models']['n_columns'])
        ...
        self.connect(self.toolButton, QtCore.SIGNAL("clicked()"), self.onUpdateFunc)
    ...
    def onLoadModelFile(self, filename=None):
        if filename is None:
            filename = mfm.widgets.open_file('Open model-file', 'link file (*.json)')
