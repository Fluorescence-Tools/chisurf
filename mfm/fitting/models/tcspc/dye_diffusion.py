...
from mfm.fluorescence.fps.dynamic import DiffusionSimulation, Dye, DyeWidget, Sticking, \
    StickingWidget, ProteinQuenching, ProteinQuenchingWidget
...
class DyeDecay(Model, Curve):
    ...
    def __init__(self, tau0=4.2, k_quench_protein=5.0, n_photons=10e6,
        ...
        self.convolve = kwargs.get('convolve', mfm.fluorescence.tcspc.convolve.Convolve(self.fit))
        ...
        self._av = None
        self._structure = mfm.structure.Structure()
        self.tau0 = tau0
...
class TransientDecayGenerator(QtGui.QWidget, DyeDecay):
    ...
    def __init__(self, fit, **kwargs):
        ...
        convolve = mfm.fluorescence.tcspc.convolve.ConvolveWidget(fit=fit, model=self, dt=fit.data.dt, **kwargs)
        ...
        if not kwargs.get('disable_fit', False):
            fitting_widget = mfm.fitting.fit.FittingControllerWidget(fit, **kwargs)
        else:
