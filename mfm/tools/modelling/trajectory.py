...
@jit
def below_min_distance(xyz, min_distance, atom_selection=np.empty(0, dtype=np.int32)):
    ...
    atoms = range(xyz.shape[1]) if atom_selection.shape[0] == 0 else atom_selection
...
class AlignTrajectoryWidget(QtGui.QWidget):
    ...
    def onOpenTrajectory(self, filename=None):
        ...
        filename = mfm.widgets.open_file('Open H5-Model file', 'H5-files (*.h5)')
...
class MDConverter(QtGui.QWidget):
    ...
    def onOpenTopology(self):
        ...
        filename = mfm.widgets.open_file('Open PDB-File', 'PDB-File (*.pdb)')
    ...
    def onLoadHDFFile(self):
        if not self.use_folder:
            ...
            filename = mfm.widgets.open_file('Open HDF-File', 'H5-File (*.h5)')
