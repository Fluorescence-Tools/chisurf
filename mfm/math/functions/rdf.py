...
@jit(nopython=True)
def worm_like_chain(rs, kappa, chain_length=0.0, normalize=True):
    """
    ...
    :param rs: numpy-array distance array
    :param kappa: float persistance length
...
def distance_between_gaussian(rs, separation, sigma, normalize=False):
    ...
    according to Oleg C:\Users\peulen\OneDrive\ChiSurf\devel\doc\20150204_Opanasyuk_About distance distribution.pdf

    r/R * (normal(R, s, r) - normal(-R, s, r)

    :param rs: array of distances rda_axis
    :param separation: Separation between the two Gaussians
    :param sigma: Width of the two Gaussians
    :param normalize: boolean if True normalize sum to unity
    :return:
    """
    if separation > 0:
