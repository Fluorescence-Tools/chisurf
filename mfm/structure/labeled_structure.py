...
import mfm.structure    >>> structure = mfm.structure.Structure('./sample_data/modelling/pdb_files/hGBP1_closed.pdb')
    ...
    kappa2 = kwargs.get('forster_radius', mfm.settings['fret']['kappa2'])
    tau0 = kwargs.get('tau0', mfm.settings['fret']['tau0'])
...
import mfm.structure    >>> structure_1 = mfm.structure.Structure('./sample_data/modelling/trajectory/h5-file/steps/0_major.pdb')
    ...
    if isinstance(convolve, mfm.fluorescence.tcspc.convolve.Convolve):
        decay = convolve.convolve(lifetime_spectrum=lifetime_spectrum, **kwargs)
    else:
...
class LabeledStructure(Structure):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.forster_radius = kwargs.get('forster_radius', mfm.settings['fret']['forster_radius'])
        self.tau0 = kwargs.get('tau0', mfm.settings['fret']['tau0'])
        self.kappa2 = kwargs.get('kappa2', mfm.settings['fret']['kappa2'])
