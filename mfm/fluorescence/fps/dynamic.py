...
import mfm
from mfm.structure import count_atoms


ne.set_num_threads(mfm.settings['n_threads'])
...
def simulate_trajectory(d, ds, dg, t_max, t_step, diffusion_coefficient, slow_fact):
    """
...
class DiffusionSimulationParameter(AggregatedParameters):
    ...
    def __init__(self, t_max=2000, t_step=0.05, n_simulations=4):
...
class DiffusionSimulation(object):
    ...
    def __init__(self, dye, quenching_parameter, diffusion_simulation_parameter, **kwargs):
    ...
    @property
    def collisions(self):
        return float(self.collided.sum()) / self.collided.shape[0]
    ...
    def save(self, filename, mode='h5', **kwargs):
        if mode == 'h5':
    ...
    def run(self, **kwargs):
        ...
        slow_fact = kwargs.get('slow_fact', self.dye.sticking.slow_fact)
        diffusion_coefficient = kwargs.get('diffusion_coefficient', self.dye.diffusion_map)

        t_max = kwargs.get('t_max', self.simulation_parameter.t_max)
        t_step = kwargs.get('t_step', self.simulation_parameter.t_step)
        ...
        av = self.dye.get_av()
        self._av = av
        traj, a, n_accepted, n_rejected = simulate_trajectory(av.density,
                                                              av.density_slow,
                                                              av.dg,
                                                              slow_fact=slow_fact,
                                                              t_step=t_step,
...
class Dye(AggregatedParameters):
    ...
    def dye_definition(self):
        return mfm.fluorescence.fps.dye_definition[self.dye_name]
    ...
    def get_av(self, **kwargs):
        ...
        av = mfm.fluorescence.fps.ACV(structure=structure,
...
class Sticking(AggregatedParameters):
...
class ProteinQuenching(AggregatedParameters):
    ...
    def __str__(self):
        s = AggregatedParameters.__str__(self)
        s += "\tstructure: %s\n" % self.structure.name
        s += "\tquencher:\n"
...
class ProteinQuenchingWidget(ProteinQuenching, QtGui.QGroupBox):
    @property
    def quencher(self):
        p = OrderedDict()
    ...
    def __init__(self, **kwargs):
        ...
        self.lineEdit_6 = QtGui.QLineEdit()
...
class DyeWidget(Dye, QtGui.QGroupBox):
    ...
    def __init__(self, **kwargs):
        QtGui.QGroupBox.__init__(self)
        self.dye_select = QtGui.QComboBox()
        ...
        self.connect(self.dye_select, QtCore.SIGNAL("currentIndexChanged(int)"), self.update_parameter)
        self.update_parameter()


class StickingWidget(Sticking, QtGui.QGroupBox):
    ...
    def __init__(self, **kwargs):
        ...
        self.setLayout(layout)
        self.radioButton = QtGui.QRadioButton('Surface')
        self.radioButton_2 = QtGui.QRadioButton('Quencher')
