...
import mfm
from mfm.fluorescence import kappasq, _general, elte2, e1tn
from mfm.parameter import AggregatedParameters, FittingParameter, FittingParameterWidget
...
def kappa(donor_dipole, acceptor_dipole):
    ...
    """
    return _general.kappa2_distance(donor_dipole[0], donor_dipole[1], acceptor_dipole[0], acceptor_dipole[1])
...
class Anisotropy(AggregatedParameters):
    ...
    def add_rotation(self, **kwargs):
        ...
        b = FittingParameter(lb=lb, ub=ub,
                             value=rho_value,
                             name='b(%s,%i)' % (self.short, len(self) + 1),
                             fixed=fixed,
                             bounds_on=bound_on)
        rho = FittingParameter(lb=lb, ub=ub,
                               value=b_value,
                               name='rho(%s,%i)' % (self.short, len(self) + 1),
    ...
    def __init__(self, **kwargs):
        ...
        self._rhos = []
        self._bs = []
        self._polarization_type = kwargs.get('polarization', mfm.settings['tcspc']['polarization'])
...
class AnisotropyWidget(Anisotropy, QtGui.QWidget):
    ...
    def __init__(self, **kwargs):
        ...
        l = QtGui.QHBoxLayout()
        self._r0 = self._r0.widget(layout=l, text='r0', fixed=True)
        self._g = self._g.widget(layout=l, text='g', decimals=2, fixed=True)
        self.lh.addLayout(l)

        l = QtGui.QHBoxLayout()
        self._l1 = self._l1.widget(layout=l, decimals=4, fixed=True, text='l1')
        self._l2 = self._l2.widget(layout=l, decimals=4, fixed=True, text='l2')
        self.lh.addLayout(l)
    ...
    def add_rotation(self, **kwargs):
        ...
        l.setSpacing(0)
        self.lh.addLayout(l)
        rho = FittingParameterWidget(name='rho_%i' % (len(self) + 1), value=rho, layout=l,
                                     model=self.model, decimals=2)
        x = FittingParameterWidget(name='b_%i' % (len(self) + 1), value=b, layout=l,
                                   model=self.model, decimals=2)
        self._rhos.append(rho)
        self._bs.append(x)
