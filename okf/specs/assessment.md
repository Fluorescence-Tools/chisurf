...
|----|-----|-----|-----------|---------|--------|
| [SV-01](#sv-01) | S1 | SV | Server | `chisurf.gui` imported inside the Qt-free server | VERIFIED |
...
| [BUG-01](#bug-01) | S1 | BUG | Core | `NCurve(d=None)` dead branch → `np.copy(None)` | VERIFIED |
| [BUG-02](#bug-02) | S1 | BUG | Server | `fit_select` defined twice in `fits.py` (second shadows first) | VERIFIED |
| [BUG-03](#bug-03) | S1 | BUG | Server | `model_component_remove` references undefined `component_type` → `NameError` | VERIFIED |
| [BUG-04](#bug-04) | S2 | BUG | Core | `@abc.abstractmethod` not enforced: `Base(object)` has no `ABCMeta` | VERIFIED |
| [DATA-01](#data-01) | S1 | DATA | Plugins | **3** manifests fail validation and are silently dropped by `load_manifest()` | VERIFIED |
...
20 findings: 8 VERIFIED, 12 REPORTED. 5×S1, 11×S2, 4×S3.
...
### INC-03
**S3 · Legacy method aliases coexist with the canonical ones.** [rpc steering](rpc.md#steering-notes), [mfdb steering](mfdb.md#steering-notes). Flat `list_datasets`/`run_fit` vs namespaced `dataset.*`/`fit.*`; MFDB `mfdb.*` (73) vs `mfdb.v1.*` (40); plus the still-present `sample_database` compat plugin. → Deprecate on a schedule and remove.
...
1. Land the **S1** items first — they are correctness/import failures with tiny, local fixes (BUG-01/02/03, SV-01, DATA-01), each independently shippable.
2. Add the guardrails that would have caught them: manifest validation in discovery (DATA-01), a duplicate-handler-name test (BUG-02), a "no `chisurf.gui` import under `chisurf/server/`" import-lint (SV-01), and a published-vs-declared event-topic assertion (SV-05).
