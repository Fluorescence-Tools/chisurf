...
- **Cache locality + narrow integers.** The engine is memory-bandwidth-bound, so `α`/`scale` are kept in small **per-thread scratch buffers** (sized to the longest burst, reused across bursts, hot in L1/L2) rather than a shared `(N, n)` array — removing the largest DRAM stream, flattening per-photon cost across dataset size (no large-`N` cliff), and cutting peak memory by tens of MB. The remaining per-photon reads (`streams`, `gap_slot`) are stored in the narrowest ints (`int8`/`int16`), which further cuts bandwidth (~40 % faster at `n=2`, where the loop is most memory-bound).
