...
The incumbent is a mature MATLAB-based integrated suite for quantitative
fluorescence microscopy and spectroscopy — FCS/FCCS/pCF, smFRET/MFD, phasor-FLIM,
and image-correlation (ICS/RICS/tICS/N&B). It is a large codebase (~730 files
across ~13 GUI apps), maintained for recent MATLAB releases, and available as a
local read-only checkout under `thirdparty/`.
...
*how* to sub-PRDs. It does not itself change code. Related work it builds on:
PRD-38 (model/view-spec split), PRD-40 (declarative dataset editors), PRD-28
(ndxplorer burst), PRD-04 (burst pipeline), PRD-26 (model-driven data layer).

# Cross-cutting mandate — AutoForm + JSON for all new work
...
- We do **not** port the incumbent's MATLAB GUIs to Qt — we re-express each
  capability as a data-driven view spec over a reusable compute core
  (tttrlib / chinet / chisurf.core).
...
### Data-browser hub (data browser, correlation, microtime, image, PIE config)
...
### Burst/smFRET (MFD) browser
| Incumbent capability | Status | ChiSurf location / note |
|---|---|---|
| Burst search & selection | PARITY+ | `burst_selection`: BOCPD/Kalman/CUSUM (incumbent is threshold-only) |
...
| 2-photon FCS model | ABSENT | small add → PRD-49 backlog |
...
| IRF conv. (Gauss/Gamma), MLE, MCMC error | PARITY+ | `irf_estimator`, `fitting/sample.py`; plus MaxEnt/LLTF the incumbent lacks |
...
### Phasor-FLIM particle detection / viewer
| Incumbent capability | Status | ChiSurf location / note |
|---|---|---|
...
- **Structural-FRET integration the incumbent has essentially none of:**
  accessible-volume modelling (`fps_json_editor`, IMP.bff → MRC densities),
  FRET-restrained rigid-body **docking & library screening** (`fret_docking`,
...
- **Modern burst detection:** Bayesian (BOCPD), Kalman, CUSUM change-point search
  vs. the incumbent's fixed thresholds.
...
provenance-tracked — i.e. we surpass the incumbent on the way to matching it.**
...
| Gap | What the incumbent does | Reuse in ChiSurf | Effort | Owner |
|---|---|---|---|---|
...
- Not reimplementing the incumbent's MATLAB UI or shipping a compiled MATLAB-free clone.
- Not a single monolithic data-browser "hub" window — ChiSurf stays plugin/AutoForm-driven.
- No new bespoke Qt widgets for analysis UIs (see the AutoForm mandate above).
