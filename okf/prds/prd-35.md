...
# Status
Planned (Proposed). The light path simulator currently stores optical path
presets as raw JSON files on disk (`~/.chisurf/presets/lightpath_optical/`); the
JSON-on-disk code path (Phase 0) is the approved interim and is done. The MFDB
write path, provenance join table, and offline fallback are future phases.
Complementary to, not a replacement for, the detailed instrument/optical-channel
schema.
