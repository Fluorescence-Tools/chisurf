...
class ProteinMCWorker(object):
    ...
    def __init__(self, structure, **kwargs):
        ...
        if self.output_traj_file is None:
            ...
            if self.verbose:
                print "input_pdb: %s" % structure.filename
        ...
        self.u1 = Universe()
        self.settings = kwargs.get('settings', mfm.settings['mc_settings'])
    ...
    def mc_1u(self, **kwargs):
        ...
        while i < n_iter and n_written < n_write:
            ...
            moving_aa = mrand.weighted_choice(move_map, 1)
            if move_phi:
                cPhi *= 0.0
...
if __name__ == "__main__":
    ...
    kwargs = vars(args)
    mfm.verbose = args.verbose
    if mfm.verbose:
        print "--------------------------------------"

    structure = mfm.structure.ProteinCentroid(args.pdb_file)
