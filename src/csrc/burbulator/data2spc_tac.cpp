...

{
 ...
 do {
    ...
    // Genuine 32-bit wraparound: previous value much larger than current.
    // data_T occasionally decreases between photons within a burst, which
    // is NOT a counter wraparound.  Distinguish by checking the decrease
    // exceeds half the 32-bit range.
    if (data_T[n] < data_T_prev && (data_T_prev - data_T[n]) > 2147483648UL)
        t_offset += tw * 4294967296L; // overflow
