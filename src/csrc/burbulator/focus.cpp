...
// Portability: uses fmin() / fabs() from <math.h> instead of MSVC __min / abs.

#include<math.h>
#include<stdlib.h>

static const double sqrt_8 = sqrt(8.);
...
{
 Iex = (fabs(x)<w0) && (fabs(y)<w0) && (fabs(z)<z0);
 return Iex;
}
...
double focus_gausslorentz_cyl(double x, double y, double z, double& Iex)
{
 ...
 return Iex * ((x*x+y*y<Rph*Rph) && (fabs(z)<z0_CEF));
}
